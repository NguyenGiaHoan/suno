...
class Cafe extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->auth == null || !in_array(21, $this->auth['group_permission']))
            ...
            $data['data']['area'] = $area;
            $table = $this->db
                ->select('cms_table.ID,table_name,area_name,table_status')
                ->from('area')
        ...
        if (isset($product) && count($product) != 0) {
            ...
            <tr id="spinner-<?php echo $product['ID']; ?>" data-id="<?php echo $product['ID']; ?>">
                ...
                <td class="text-center">
                    ...
                    <script>
 $('#spinner-<?php echo $product['ID']; ?> .spinner input').on('input', function() {
        // Xử lý khi giá trị của ô input thay đổi
        var currentValue = parseInt($(this).val(), 10);
        if (isNaN(currentValue) || currentValue < 1) {
            // Nếu giá trị không phải là số hợp lệ hoặc nhỏ hơn 1, đặt giá trị là 1
            $(this).val(1);
        } else {
            // Nếu là số hợp lệ, giữ nguyên giá trị
            $(this).val(currentValue);
        }
        // Gọi hàm cms_load_infor_order() để cập nhật thông tin đơn hàng
        cms_load_infor_order();
    });

    $('.spinner .btn:last-of-type').on('click', function() {
        // Tăng giá trị số lượng khi nút '+' được nhấn
        var currentValue = parseInt($('#spinner-<?php echo $product['ID']; ?> .spinner input').val(), 10);
        $(this).parents('#spinner-<?php echo $product['ID']; ?> .spinner').find('input').val(currentValue + 1);
        // Gọi hàm cms_load_infor_order() để cập nhật thông tin đơn hàng
        cms_load_infor_order();
    });

    $('.spinner .btn:first-of-type').on('click', function() {
        // Giảm giá trị số lượng khi nút '-' được nhấn
        var currentValue = parseInt($('#spinner-<?php echo $product['ID']; ?> .spinner input').val(), 10);
        var newValue = currentValue - 1 < 1 ? 1 : currentValue - 1;
        $(this).parents('#spinner-<?php echo $product['ID']; ?> .spinner').find('input').val(newValue);
        // Gọi hàm cms_load_infor_order() để cập nhật thông tin đơn hàng
        cms_load_infor_order();
    });
                    </script>
    ...
    public function cms_load_pos($table_id)
    {
        ...
        $data['data']['tables'] = $this->db
            ->select('cms_table.ID as ID, table_name,area_name,table_status')
            ->from('table')
    ...
    public function cms_print_table()
    {
        if ($this->auth == null)
            ...
            if (isset($data_order) && count($data_order)) {
                ...
                foreach ($list_products as $product) {
                    if($product['printed'] < $product['quantity']){
                        $prd = cms_finding_productbyID($product['id']);
                        $quantity = $product['quantity'] - $product['printed'];
...
if (empty($data['area_name'])) {
    echo $this->messages = "Nhập tên khu vực";
    return;
}

// Kiểm tra số bàn
if (empty($data['number_table']) || $data['number_table'] < 1) {
    echo $this->messages = "Nhập số bàn cho khu vực";
    return;
}           

            // Kiểm tra xem khu vực đã tồn tại hay chưa
