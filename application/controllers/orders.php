...
class Orders extends CI_Controller
{
    ...
    public function cms_print_order()
    {
        ...
        $data_template['content'] = str_replace("{Ngay_Xuat}", ($data_order['sell_date'] != '0000-00-00 00:00:00') ? gmdate("H:i d/m/Y", strtotime(str_replace('-', '/', $data_order['sell_date'])) + 7 * 3600) : '-', $data_template['content']);
        ...
        if (isset($data_order) && count($data_order)) {
            ...
            foreach ($list_products as $product) {
                ...
                $detail = $detail . '<tr><td style="text-align:center;">' . $number++ . '</td><td  style="text-align:center;">' . $prd['prd_name'] .'<br>'.$product['note']. '</td><td style = "text-align:center">' . $quantity . '</td ><td style = "text-align:center">' . $prd['prd_unit_name'] . '</td ><td style = "text-align:center">' . $prd['infor'] . '</td ><td  style="text-align:center;">' . $this->cms_common->cms_encode_currency_format($product['price']) . '</td><td style="text-align:center;">' . $this->cms_common->cms_encode_currency_format($total) . '</td ></tr>';
                $detail2 = $detail2 . '
                <tr>
                    <td>' . $prd['prd_name'] .'<br>'.$product['note'].  '</td>
    ...
    public function cms_save_order_return_and_sell($store_id)
    {
        if ($this->auth['store_id'] == $store_id) {
            $input = $this->input->post('return');
            $order = $this->input->post('sell');
            $check_order = $this->db->where('ID', $input['order_id'])->from('orders')->get()->row_array();
            if (empty($check_order)) {
                echo $this->messages = "0";
                return;
            ...
            foreach ($input['detail_input'] as $item) {
                ...
                $product = $this->db->select('prd_sls,prd_origin_price')->from('products')->where('ID', $item['id'])->get()->row_array();
                $sls['prd_sls'] = $product['prd_sls'] + $item['quantity'];
                $total_price_return += ($item['price'] * $item['quantity']);
                $total_quantity += $item['quantity'];
                $this->db->where('ID', $item['id'])->update('products', $sls);
            ...
            $input['total_quantity'] = $total_quantity;
            $input['total_price'] = $total_price_return;
            $input['total_money'] = $total_price_return;
            $input['payed'] = $total_price_return;
            $input['store_id'] = $store_id;
            $input['user_init'] = $this->auth['id'];
            $input['detail_input'] = json_encode($input['detail_input']);
            ...
            $payment['notes'] = $input['notes'];
            $payment['payment_method'] = $input['payment_method'];
            $payment['total_money'] = $total_price_return;
            $payment['user_init'] = $input['user_init'];
            $this->db->insert('payment', $payment);
            ...
            $order['customer_id'] = $check_order['customer_id'];
            $order['customer_pay'] += $total_price_return;
        ...
        if ($this->auth == null)
            ...
            if (isset($order) && count($order)) {
                $detail_order = $this->db
                    ->select('cms_canreturn.ID,product_id,prd_code,prd_name,quantity,price')
                    ->from('canreturn')
    ...
    public function cms_autocomplete_products($customer_id)
    {
        $data = $this->input->get('term');
        if($customer_id==0){
            $products = $this->db
                ->select('prd_sell_price,ID,prd_name,prd_code')
                ->from('products')
            ...
            if(empty($customer) || $customer['customer_group']==0){
                ...
                $products = $this->db
                    ->select('prd_sell_price2 as prd_sell_price,ID,prd_name,prd_code')
                    ->from('products')
                    ->where('(prd_code like "%' . $data . '%" or prd_name like "%' . $data . '%") and prd_status = 1 and deleted =0 ')
                    ->get()
                    ->result_array();
    ...
    public function cms_select_product($customer_id)
    {
        ...
        if($customer_id==0){
            ...
            if(empty($customer) || $customer['customer_group']==0){
                ...
                $product = $this->db
                    ->select('products.ID,prd_code,prd_unit_name,prd_name, prd_sell_price2 as prd_sell_price, prd_image_url,prd_edit_price')
                    ->from('products')
                    ->where(['products.ID' => $id, 'deleted' => 0, 'prd_status' => 1])
                    ->join('products_unit', 'products_unit.ID=products.prd_unit_id', 'LEFT')
                    ->get()
        ...
        if (isset($product) && count($product) != 0) {
            ...
            <tr data-id="<?php echo $product['ID']; ?>">
                ...
                <td class="text-center" style="max-width: 30px;"><input style="min-width:80px;max-height: 22px;"
                                                                        type="text"
    ...
    public function cms_select_product_gift()
    {
        $id = $this->input->post('id');
        $seq = $this->input->post('seq');
        $product = $this->db
        ...
        if (isset($product) && count($product) != 0) {
            ...
            <tr data-id="<?php echo $product['ID']; ?>">
                ...
                <td class="text-center" style="max-width: 30px;"><input style="min-width:80px;max-height: 22px;"
                                                                        ...
                                                                        value="1"></td>
                <td class="text-center"><?php echo $product['prd_unit_name']; ?> </td>
                <td class="text-center"><?php echo cms_encode_currency_format($product['prd_sell_price']); ?> </td>
                <td class="text-center">0 </td>
                <td class="text-center total-money">0</td>
                <td class="text-center"><i class="fa fa-trash-o del-pro-order"></i></td>
            </tr>
            <?php
            $html = ob_get_contents();
